...
task: fetch_and_extract
input:
urls: - https://news.ycombinator.com - https://huggingface.co/papers
output_schema: - source_id: string - title: string - summary: string - key_points: string[] - url: string - keywords: string[] - quality_score: 1-5
constraints:
filter: 前沿技术/高深技术/提效技术/实用资讯
...
**摘要**：2-4 行具体概述，内容必须反映该新闻的实际内容，禁止使用"Trending on HN"、"Tech community discussion"等模板化语言。

**要点**：

1. 具体要点一
2. 具体要点二
3. 具体要点三（每个要点必须不同，禁止重复）

**来源**：[具体链接文本](URL)

**关键词**：`keyword1` `keyword2` `keyword3`（关键词必须与内容相关，禁止使用 `HN` `tech` `discussion` 等通用标签）
